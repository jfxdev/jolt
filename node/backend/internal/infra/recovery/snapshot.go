...
var ErrInstanceActive = errors.New("jolt node is running")

const lockName = ".jolt-node.lock"
...
func CreateSnapshot(ctx context.Context, dataDir, keysDir, output string) (Manifest, error) {
	dataDir, keysDir, output = filepath.Clean(dataDir), filepath.Clean(keysDir), filepath.Clean(output)
	if dataDir == keysDir || inside(dataDir, keysDir) || inside(keysDir, dataDir) {
		return Manifest{}, errors.New("data and keys directories must not overlap")
	}
	if output == "." || output == "" {
		return Manifest{}, errors.New("snapshot output is required")
	}
	if inside(output, dataDir) || inside(output, keysDir) {
		return Manifest{}, errors.New("snapshot output must be outside data and keys directories")
	}
	...
	stage, err := os.MkdirTemp("", "jolt-node-snapshot-*")
	if err != nil {
		return Manifest{}, err
	}
	...
	stageData, stageKeys := filepath.Join(stage, "data"), filepath.Join(stage, "keys")
	if err := os.MkdirAll(stageData, 0o700); err != nil {
		return Manifest{}, err
	}
	integrity, err := snapshotSQLite(ctx, filepath.Join(dataDir, "jolt.db"), filepath.Join(stageData, "jolt.db"))
	if err != nil {
		return Manifest{}, err
	}
	if err := copyTree(dataDir, stageData, map[string]bool{
		"jolt.db": true, "jolt.db-wal": true, "jolt.db-shm": true, lockName: true,
	}); err != nil {
		return Manifest{}, fmt.Errorf("stage node data: %w", err)
	}
	if err := copyTree(keysDir, stageKeys, nil); err != nil {
		return Manifest{}, fmt.Errorf("stage node keys: %w", err)
	}
	identity, err := joltcrypto.LoadOrCreate(stageKeys)
	if err != nil {
		return Manifest{}, fmt.Errorf("validate snapshot identity: %w", err)
	}
	...
	manifest := Manifest{
		Version: 1, Kind: "jolt_node", CreatedAt: time.Now().UTC(),
		DatabaseIntegrity: integrity, NodeID: identity.NodeID,
		Fingerprint: identity.Fingerprint, Files: files,
	}
...
func snapshotSQLite(ctx context.Context, source, destination string) (string, error) {
...
func copyTree(source, destination string, excluded map[string]bool) error {
	...
	return filepath.WalkDir(source, func(path string, entry fs.DirEntry, walkErr error) error {
		...
		if excluded != nil && excluded[relative] {
			if entry.IsDir() {
...
func writeArchiveAtomic(stage, output string, manifest Manifest) error {
	...
	temp, err := os.CreateTemp(parent, ".jolt-node-snapshot-*.tmp")
	if err != nil {
		return err
	}
	tempName := temp.Name()
